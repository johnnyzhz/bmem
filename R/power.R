...
power.basic<-function(model, nobs=100, nrep=1000, alpha=.95, skewness=NULL, kurtosis=NULL, ovnames=NULL, se="default", estimator="default", parallel="no", ncore=1, ...){
  ...
  res <- if (ncore > 1L && parallel != "no") {
    ...
    snowfall::sfLapply(seq_len(RR), runonce)
...
power.robust<-function(model, nobs=100, nrep=1000, alpha=.95, skewness=NULL, kurtosis=NULL, ovnames=NULL, parallel="no", ncore=1, ...){
  if (missing(model)) stop("A model is needed.")
...
power.boot<-function(model, nobs=100, nrep=1000, nboot=1000, alpha=.95, skewness=NULL, kurtosis=NULL, ovnames=NULL, ci='default', boot.type='default', se="default", estimator="default", parallel="no", ncore=1, ...){		
  if (missing(model)) stop("A model is needed.")	
